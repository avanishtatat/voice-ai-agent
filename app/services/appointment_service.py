...
def book_appointment_service(payload, db):
    # 1. Validate patient 
...
def cancel_appointment_service(payload, db): 
...
def reschedule_appointment_service(payload, db):
    # 1. Validate appointment 
    appointment = db.query(Appointment).filter(Appointment.id == payload.appointment_id).first()
    if not appointment:
        raise HTTPException(status_code=404, detail="Appointment not found")
...
def get_available_slots_service(doctor_id, date, db):
    # 1. Validate doctor 
